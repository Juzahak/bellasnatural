...
const getAllProducts_limit = async (req, res) => {
  try {
    console.log("all the products");
    let productData = await executeQuery("select `id`,`name`,`category`,`subcategory`,`price`,`image1`,`featured`,`active` from products where active = 1", []);
...
const saveProducts_limit = async (req, res) => {
  ...
  const deleteProducts_limitById = async (req, res, next) => {
    let id = req.query.id;
    try {
      let ProductData = await executeQuery(
  ...
  const updateProducts_limit = async (req, res) => {
    let id = req.query.id;
  ...
  const getProducts_limitById = async (req, res, next) => {
    let id = req.query.id;
    try {
